...
| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-24-01 | arXiv:2609.22537 — EvidenT: 이종 엔터프라이즈 RAG 인용 이탈·비지원 콘텐츠 제거 파이프라인 (EMNLP 2026 Industry Track, 2026-09-18) | 엔터프라이즈·사내지식 (증거 근거 보장, 인용 이탈 제거, 재학습 불필요 드롭인 파이프라인) | 01-엔터프라이즈-사내지식.md |
| GL-2026-09-24-02 | arXiv:2609.25469 — RAG-NAROK: 검색 인식 블랙박스 RAG 출처-특정 반박 오염 공격 (Southern Illinois University, 2026-09-21) | 프로덕션 아키텍처·보안 (검색 인식 포이즈닝, Shadow RAG 관측 루프, Anchor-Specific Refutation) | 02-프로덕션-아키텍처.md |
| GL-2026-09-24-03 | arXiv:2609.27579 — ELBE RAG: 입자가속기 제어 시스템 이질적 운영 데이터 통합 RAG (HZDR, 2026-09) | 산업별·과학·연구 (로그북+시계열+매뉴얼 통합, 도메인 적응 임베딩, 오퍼레이터 지향) | 04-산업별-사례.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (3개 이상 독립 출처)
  1. **arXiv:2609.22537 (EvidenT)** [(저자 미확인); EMNLP 2026 Industry Track]: 이종 엔터프라이즈 문서 RAG에서 인용 이탈·비지원 콘텐츠를 재학습 없이 제거하는 경량 파이프라인. 구조화 패시지 추출 + 결정론적 어휘 정렬. 약 500개 실사용 쿼리에서 gold-source hit rate +29%, 비검색 URL 인용 0건. → `01-엔터프라이즈-사내지식.md` RAFT 다음(## 이 도메인의 공통 패턴 앞)에 추가
  2. **arXiv:2609.25469 (RAG-NAROK)** [Abdullahil Kafi, Alvi Ataur Khalil; TITANS Lab, Southern Illinois University]: RAG 파이프라인 투명성을 역이용해 검색 동역학 관측(정찰+Shadow RAG 루프) 후 Anchor-Specific Refutation 문서를 동적으로 생성하는 검색 인식 블랙박스 공격. 법률·기술·금융 3개 도메인 평가. → `02-프로덕션-아키텍처.md` Re:CAP 다음(## 한국 환경 특이점 앞)에 추가
  3. **arXiv:2609.27579 (ELBE RAG)** [N. Mirian; HZDR, 독일]: 전자 선형 가속기 ELBE 제어 시스템에서 전자 로그북·기계 아카이브 시계열 데이터·서브시스템 매뉴얼을 단일 RAG 프레임워크로 통합 인덱싱. 도메인 적응 임베딩 + 오퍼레이터 지향 구조화 응답 + 추적 가능한 출처 참조. → `04-산업별-사례.md` PlanSightRAG 다음(### 한국 앞)에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-24 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (검색 범위 내 최근 7일 한국 기관/기업 신규 RAG 사례 미발견; 네이버·카카오·토스·SKT·KT·우아한형제들·LY Corp·삼성SDS·LG CNS·금융권·의료기관·로앤컴퍼니 등 다수 쿼리 시도; 우아콘 2026은 10월 28일 예정으로 아직 미개최)
...
| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-22-01 | 포티투마루(42MARU) LLM42·RAG42 정부 AI 제품·서비스 확인제 동시 인증 (2026-09-16) | 공공·행정 (RAG 솔루션 공공조달 인증, 온프레미스 폐쇄망 지원) | 04-산업별-사례.md |
| GL-2026-09-22-02 | arXiv:2609.16818 — InceptionRAG: 단일 문서 탐지 우회 분산 잠복 패시지 체인 RAG 포이즈닝 공격 (ACM CCS 2026, 2026-09-15) | 프로덕션 아키텍처 (RAG 보안·포이즈닝·분산 공격) | 02-프로덕션-아키텍처.md |
| GL-2026-09-22-03 | arXiv:2609.12190 — 과학 코드 이해를 위한 완전 로컬 RAG 에이전트: 오프라인 집약·온라인 경량 분리 (2026-09-10) | 에이전트·툴유즈 (로컬 소형 모델 RAG, 오프라인/온라인 단계 분리) | 03-에이전트-툴유즈-MCP.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (2~6개 이상 독립 출처)
  1. **포티투마루 RAG42 인증** [공식 공고 2026-09-16]: RAG42 v.1.3 + LLM42 v.1.1 인공지능 기본법 시행령 기반 정부 AI 확인제 동시 인증. 공공조달 MAS 완화·신인도 가점·납품실적 면제 혜택. → `04-산업별-사례.md` 그리드원 × 한국수자원공사 다음(### 글로벌 앞)에 추가
  2. **arXiv:2609.16818 (InceptionRAG)** [Jiachang Zhang, Min Chen, Xiao Ren, Zhenyong Zhang, Yuanchao Shu, Yunjun Gao, Zhikun Zhang; ACM CCS '26]: 악성 페이로드를 무해한 잠복 패시지 체인으로 분산, 단일 문서 탐지 방어 우회. Micro-Collaborative Poisoning(2609.21573)의 상보 공격 유형. → `02-프로덕션-아키텍처.md` Micro-Collaborative Poisoning 다음(## 한국 환경 특이점 앞)에 추가
  3. **arXiv:2609.12190 (Scientific Code RAG)** [Aaron Nobile, Andreas Adelmann, Mohsen Sadr]: 오프라인 집약(AST 파싱·그래프·LLM 설명 생성·임베딩) + 온라인 경량(검색·소형 모델 답변) 분리. IPPL C++ 코드베이스 100문항 11카테고리 벤치마크. → `03-에이전트-툴유즈-MCP.md` FootprintRAG 다음(## 이 도메인의 공통 패턴 앞)에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-22 섹션 신설, 파일 최상단)
- **한국 사례**: 1건 (포티투마루 RAG42 정부 AI 확인제 인증, 2026-09-16, Nate News 등 6개 이상 독립 출처)
- **검증 결과**: URL 200 OK: 0/3건(전면 WebFetch 차단) · snippet-verified: 3/3건 · 단언 톤다운: 1건(포티투마루 "1호" 표현 삭제 후 "인증 이력 보유"로 완화) · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 10건 시도 → 3건 채택
...
| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-16-01 | arXiv:2609.15800 — SCORE: 에이전틱 비주얼 RAG, 텍스트 원장 + 바운딩박스 줌인 + 멀티턴 RL (Soochow Univ. · Baidu, 2026-09-14) | 에이전트·툴유즈 (Agentic Visual RAG, 멀티턴 RL VLM) | 03-에이전트-툴유즈-MCP.md |
| GL-2026-09-16-02 | arXiv:2609.15830 — CiteGuard-RAG: 인용 검증 중심 법률 QA, 99.1% 검색·98.3% 인용 유효성 (Western Michigan Univ., 2026-09-14) | 산업별·법률 (인용 강제·문장 단위 근거 검증) | 04-산업별-사례.md |
| GL-2026-09-16-03 | arXiv:2609.16095 — RAG-CT: 프롬프트 분포 스캐닝으로 PII 프라이버시 방어, 경량 쿼리 단계 탐지 (2026-09-14) | 프로덕션 아키텍처 (보안·PII 방어·경량 탐지) | 02-프로덕션-아키텍처.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2609.15800 (SCORE)** [Soochow University · Baidu Inc.]: Yucheng Shen 외 6인. 희소 비주얼 증거 탐색을 위한 텍스트 원장 메모리 + 조대-세밀 바운딩박스 줌인 + 멀티턴 RL VLM 훈련 결합. → `03-에이전트-툴유즈-MCP.md` SearchAtlas 다음(## 이 도메인의 공통 패턴 앞)에 추가
  2. **arXiv:2609.15830 (CiteGuard-RAG)** [Western Michigan University]: Sumit Barua 외 4인. 하이브리드 검색 + 인용 제약 생성 + 문장 단위 근거 검증 + 단일 패스 재생성. housing-law/PrivacyQA/CUAD 400 질의에서 99.1% 검색·98.3% 근거 답변·98.3% 인용 유효성·환각 0건. → `04-산업별-사례.md` 법률 RAG 환각 평가 다음(### 한국 앞)에 추가
  3. **arXiv:2609.16095 (RAG-CT)** [기관 미확인]: Xingyu Lyu 외 5인. 엔트로피 + 마진 분포 스캐닝으로 PII 추출 쿼리 탐지. LLM/리트리버 수정 없이 경량 쿼리-단계 방어층 삽입. 4가지 공격 × 4가지 방어 기준선 × 2개 데이터셋 실험. → `02-프로덕션-아키텍처.md` RAGSieve 다음에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-16 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (검색 범위 내 최근 7일 한국 기관/기업 신규 RAG 사례 미발견)
- **검증 결과**: URL 200 OK: 0/3건(전면 WebFetch 차단) · snippet-verified: 3/3건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 12건 시도 → 3건 채택
...
| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-15-01 | arXiv:2609.11390 — VikingRAG: 디렉터리 인식 구조화 문서 RAG 토큰 비용 대폭 절감 (ByteDance Volcengine, 2026-09-10) | 프로덕션 아키텍처 (토큰 효율·구조화 문서 RAG 서빙) | 02-프로덕션-아키텍처.md |
| GL-2026-09-15-02 | arXiv:2609.14245 — 귀속-압축 프론티어: RAG 컨텍스트 압축과 인용 귀속 품질의 트레이드오프 (2026-09-13) | 프로덕션 아키텍처 (압축 방법론·귀속 세탁·인용 추적) | 02-프로덕션-아키텍처.md |
| GL-2026-09-15-03 | arXiv:2609.12437 — Beyond the Query: 검색 신호가 멀티모달 RAG 라우팅을 개선하는가? (부정적 결과, 2026-09-11) | 프로덕션 아키텍처 (적응형 RAG·멀티모달 라우팅) | 02-프로덕션-아키텍처.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2609.11390 (VikingRAG)** [ByteDance Volcengine]: Peiyuan Gao 외 7인. 디렉터리 인식 시맨틱 DB + 경험 엣지 재활용 + 적응형 에스컬레이션으로 SOTA 대비 토큰 11.6~51.9% 수준 유지, 트레이스 재활용 시 5.1~32.5%로 추가 절감. → `02-프로덕션-아키텍처.md` GeneralQPP 다음(## 한국 환경 특이점 앞)에 추가
  2. **arXiv:2609.14245 (귀속-압축 프론티어)** [Deepanshu Mody]: 귀속 세탁(attribution laundering) 개념 제안. ASQA 0.25 예산에서 RECOMP 압축기 요약 대비 precision 0.86 vs 원문 스팬 대비 precision 0.12 격차 측정. → `02-프로덕션-아키텍처.md` VikingRAG 다음에 추가
  3. **arXiv:2609.12437 (Beyond the Query)** [Qiaomu Li, Qiuyuan Zhang, Nong Ming]: 문서·오디오·비디오 3개 모달리티 RAG에서 검색 신호가 쿼리 전용 라우터 대비 신뢰할 만한 개선 없음 실증(부정적 결과). → `02-프로덕션-아키텍처.md` 귀속-압축 프론티어 다음에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-15 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (검색 범위 내 최근 7일 한국 기관/기업 신규 RAG 사례 미발견)
- **검증 결과**: URL 200 OK: 0/3건(전면 WebFetch 차단) · snippet-verified: 3/3건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 15건 시도 → 3건 채택
...
| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-12-01 | arXiv:2609.11209 — REVA: 주목 이력 기반 재사용 가능한 증거 뷰 RAG 서빙 최적화 (ICDM 2026) | 프로덕션 아키텍처 (RAG 서빙 효율화, 컨텍스트 압축) | 02-프로덕션-아키텍처.md |
| GL-2026-09-12-02 | arXiv:2609.08887 — Q2D-Web: Perplexity 에이전틱 RAG 대규모 검색 벤치마크 (190M 문서, 10개 언어) | 에이전트·툴유즈 (에이전틱 RAG 검색 평가) | 03-에이전트-툴유즈-MCP.md |

- **검증 결과**: 2건 전원 5점 자가검증 통과; snippet-verified (5개 이상 독립 출처)
  1. **arXiv:2609.11209 (REVA)**: Tuan Nguyen 외 5인. 생성기 과거 주목 트레이스 → 문서 키 기반 점수 스토어 구축 → 예산 맞춤 증거 뷰 온라인 렌더링. 압축 오버헤드 5.3~15.6배 절감, 품질 +1.0~5.8포인트, 추가 레이턴시 <40ms. ICDM 2026. → `02-프로덕션-아키텍처.md` RAG-Safety-Bench 다음(## 한국 환경 특이점 앞)에 추가
  2. **arXiv:2609.08887 (Q2D-Web)**: Maximilian Schall 외 6인 (Perplexity). 190M 웹 문서 + 69,721 에이전트 재작성 쿼리 + 10개 언어 + 3종 관련성 판단셋. 13개 검색기 벤치마크. 인간 쿼리와 에이전트 재작성 쿼리의 분포 괴리 최초 대규모 정량화. 한국어 포함 다국어 지원. → `03-에이전트-툴유즈-MCP.md` Verbal-R3 다음(## 이 도메인의 공통 패턴 앞)에 추가

- `sources.md`에 2개 출처 추가 (## 2026-09-12 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (검색 범위 내 최근 7일 한국 기관/기업 신규 RAG 사례 미발견)
- **검증 결과**: URL 200 OK: 0/2건(전면 WebFetch 차단) · snippet-verified: 2/2건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 15건 시도 → 2건 채택
...
| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| KR-2026-09-07-01 | arXiv:2609.04981 — APT-RAG: 고려대·한양대·현대자동차 적응적 계획 + 위상 인식 증거 수집 트리형 RAG | 에이전트·툴유즈·MCP (한국 산학협력) | 03-에이전트-툴유즈-MCP.md |
| GL-2026-09-07-01 | arXiv:2609.05152 — DEX-Comp: 소프트 컨텍스트 압축 2단계 훈련(16× 압축·최대 24× 추론 속도) | 프로덕션 아키텍처 (추론 효율) | 02-프로덕션-아키텍처.md |

- **검증 결과**: 2건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2609.04981 (2026-09-04)** [한국]: 고려대학교·한양대학교·현대자동차 공동. APT-RAG: 증거 집약적 QA에서 고정 트리 구조의 경직성·위상 무지 수집 한계 극복. 적응적 계획으로 트리 동적 확장 + 형제 노드 재사용·직접 검색·자식→부모 집계 3메커니즘. → `03-에이전트-툴유즈-MCP.md` CodePoisonRAG 다음(## 이 도메인의 공통 패턴 앞)에 추가
  2. **arXiv:2609.05152 (2026-09-04)** [글로벌]: Shandong University + Bloomberg + Leiden University. DEX-Comp 2단계 훈련(Pure Distillation + Hard Exploration RL)으로 16× 압축·4×~24× 추론 가속. 미압축 RAG 실패 쿼리에서 압축 RAG가 역전 성공 실증. → `02-프로덕션-아키텍처.md` DocuSearch 다음(## 한국 환경 특이점 앞)에 추가

- `sources.md`에 2개 출처 추가 (## 2026-09-07 섹션 신설, 파일 최상단)
- **한국 사례**: 1건 (arXiv:2609.04981 — 고려대·한양대·현대자동차 APT-RAG)
- **검증 결과**: URL 200 OK: 0/2건(전면 WebFetch 차단) · snippet-verified: 2/2건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 8건 시도 → 2건 채택

## 2026-09-06 (일일 루프 #82)
- **신규 사례 3건** (WebFetch 전면 차단 환경, snippet-verified 전건; 한국 사례: 1건 — 로앤컴퍼니 렉스팬드 AI 아시아 9개 법역 법률 RAG 플랫폼)

| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| KR-2026-09-06-01 | 로앤컴퍼니 렉스팬드 AI — 슈퍼로이어 기반 아시아 9개 법역 다국어 법률 RAG | 법률 > 한국 (글로벌 확장) | 04-산업별-사례.md |
| GL-2026-09-06-01 | arXiv:2609.00761 — Agent-Enhanced Heterogeneous Graph RAG for Academic QA (WWW 2026) | 에이전트·툴유즈·MCP (이종 학술 그래프) | 03-에이전트-툴유즈-MCP.md |
| GL-2026-09-06-02 | arXiv:2609.02774 — CodePoisonRAG: 코드 생성 RAG 지식 기반 오염 공격 | 에이전트·보안 (코드 RAG 신뢰 경계) | 03-에이전트-툴유즈-MCP.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **로앤컴퍼니 렉스팬드 AI (2026-09-03)** [한국]: 슈퍼로이어 기반. 아시아 9개 법역·8개 언어 지원 법률 AI 플랫폼. RAG + 팩트체커(인용 원문 하이퍼링크·환각 억제) + 제로 데이터 리텐션 + AWS + 내부망 프라이빗 링크. → `04-산업별-사례.md` 법률 > 한국, 율촌 × BHSN 다음(### 특이 패턴 앞)에 추가
  2. **arXiv:2609.00761 (2026-09-01)** [글로벌]: ACM WWW 2026. OpenAlex + DBLP 이종 학술 그래프 기반 에이전틱 RAG. 쿼리 인식 검색 에이전트 + 충분성 인식 재랭킹 에이전트 + 그래프 팩트 검증 에이전트 3단계. 고정 전략·불완전 증거·검증 부재 세 한계 동시 해소. → `03-에이전트-툴유즈-MCP.md` ACToR 다음(## 이 도메인의 공통 패턴 앞)에 추가
  3. **arXiv:2609.02774 (2026-09-02)** [글로벌]: University of Duisburg-Essen. CWE 특화 취약점 주입 + 의미론적 오기재 조합 공격으로 RACG(코드 생성 RAG) 신뢰 경계 취약점 공략. 방어 전략 적용에도 공격 성공률 0.80~0.93. → `03-에이전트-툴유즈-MCP.md` Agent-Enhanced Heterogeneous Graph RAG 다음에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-06 일일 누적 추가 출처 섹션 신설, 파일 최상단)
- **한국 사례**: 1건 (로앤컴퍼니 렉스팬드 AI — 아시아 9개 법역 법률 AI 플랫폼)
- **검증 결과**: URL 200 OK: 0/3건(전면 WebFetch 차단) · snippet-verified: 3/3건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 10건 시도 → 3건 채택

## 2026-09-05 (일일 루프 #81)
- **신규 사례 3건** (WebFetch 전면 차단 환경, snippet-verified 전건; 한국 사례: 1건 — arXiv:2609.03454 숙명여대·KENTECH 정신건강 RAG)

| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| KR-2026-09-05-01 | arXiv:2609.03454 — 숙명여대·KENTECH: 정신건강 단일 턴 QA 선택적 RAG | 의료·헬스케어 (한국 학술) | 04-산업별-사례.md |
| GL-2026-09-05-01 | arXiv:2609.01617 — DocuSearch: 통신망 엔터프라이즈 하이브리드 RAG (KG 확장 + RRF 융합) | 프로덕션 아키텍처 (하이브리드 검색) | 02-프로덕션-아키텍처.md |
| GL-2026-09-05-02 | arXiv:2609.03874 — STAIR(IBM): 목차 기반 구조 인식 엔터프라이즈 문서 검색 | 엔터프라이즈 사내지식 (문서 구조 RAG) | 01-엔터프라이즈-사내지식.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2609.03454 (2026-09-03)** [한국]: Hyunseo Oh·Yoonhyuk Choi(숙명여대), Chong-Kwon Kim(KENTECH). 정신건강 단일 턴 QA에서 드래프트 조건부 3차원 유틸리티(심리교육·대처·구체성) + 룰 기반 안전 트리거로 선택적 검색 정책 구성. → `04-산업별-사례.md` 의료 > 한국 > VITA 다음에 추가
  2. **arXiv:2609.01617 (2026-09)** [글로벌]: Harish Saragadam, Sudhanshu Sharma, Meghana Pujari. 통신망 엔터프라이즈 DocuSearch 시스템. Qdrant+BGE-Large + BM25/SQLite FTS5 + KG 이웃 확장 → RRF(가중치 0.50/0.35/0.15, k=60). P@10 0.69, R@10 0.79, 그라운딩률 89.6% (+15%p/+16%p/+18.4%p). → `02-프로덕션-아키텍처.md` DKL 다음에 추가
  3. **arXiv:2609.03874 (2026-09-03)** [글로벌·IBM]: Vineet Kumar 외 5인(IBM Research). 목차(ToC) 전역 구조를 DSI 파인튜닝 신호로 통합. 환각률 0.05% 미만, 저자원 환경 일반화 가능. → `01-엔터프라이즈-사내지식.md` 코드베이스 RAG 섹션 다음에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-05 일일 누적 추가 출처 섹션 신설, 파일 최상단)
- **한국 사례**: 1건 (숙명여대·KENTECH 정신건강 RAG 학술 연구)
- **검증 결과**: URL 200 OK: 0/3건(전면 WebFetch 차단) · snippet-verified: 3/3건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 8건 시도 → 3건 채택

## 2026-09-04 (일일 루프 #80)
- **신규 사례 3건** (WebFetch 전면 차단 환경, snippet-verified 전건; 한국 사례: 0건 — 2026-08-28~09-04 기간 네이버·카카오·토스·SKT·KT·우아한형제들·LY Corp·삼성SDS·금융기관·의료기관 등 다수 쿼리 시도, 기수록 목록 외 신규 한국 1차 RAG 출처 미발굴)

| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-04-01 | arXiv:2609.00513 — ISO-RAG: 등방 경계 위상 제어로 그래프 RAG 시맨틱 드리프트 해소 | 프로덕션 아키텍처 (그래프 RAG 위상 노이즈 제어) | 02-프로덕션-아키텍처.md |
| GL-2026-09-04-02 | arXiv:2609.02894 — R²Adapter: 바닐라/그래프 RAG 쿼리 유형별 동적 라우팅 경량 어댑터 | 프로덕션 아키텍처 (하이브리드 RAG 라우팅) | 02-프로덕션-아키텍처.md |
| GL-2026-09-04-03 | arXiv:2609.02685 — DKL: 지식 학습과 지시 정렬 분리로 코퍼스 특화 지식 RAG 없이 주입 | 프로덕션 아키텍처 (파라메트릭 RAG 보완) | 02-프로덕션-아키텍처.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2609.00513 ISO-RAG (2026-09-01)** [글로벌]: Siyuan Zhang, Hanchen Wang, Dong Wen, Ying Zhang, Wenjie Zhang. 지역 Cheeger 비율(위상적 팽창률)로 노드별 등방 프로파일 계산 → 의사 지름길 엣지 사전 제거 → 국소화 문맥 안전 서브그래프 내 검색 제한. 쌍곡 공간(Poincaré ball) 임베딩. 학습 불필요 드롭인. → `02-프로덕션-아키텍처.md` Incremental Pooled LLM Eval 다음(## 한국 환경 특이점 앞)에 추가
  2. **arXiv:2609.02894 R²Adapter (2026-09)** [글로벌]: 중국과학원 AI 안전 국가중점실험실 + ICT(CAS) + UCAS + 메이퇀. 경량 플러그인 Routing+Rewriting Adapter로 쿼리를 바닐라 RAG와 그래프 RAG 사이 동적 배분. 3개 멀티홉 QA 벤치마크 대상 그래프 RAG 호출 최대 59% 절감, 정확도 동등 유지. 코드 공개. → `02-프로덕션-아키텍처.md` ISO-RAG 다음에 추가
  3. **arXiv:2609.02685 DKL (2026-09-02)** [글로벌]: 지식 학습과 지시 정렬 분리 훈련 패러다임. 전체 코퍼스 합성 QA 없이 텍스트 코퍼스에서 직접 지식 흡수 + 지시 따르기 능력 보존. RAG 검색 의존 줄이는 파라메트릭 지식 내재화 경로 제시. → `02-프로덕션-아키텍처.md` R²Adapter 다음에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-04 일일 누적 추가 출처 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (2026-08-28~09-04 기간 기수록 목록 외 신규 한국 1차 출처 미발굴)
- **검증 결과**: URL 200 OK: 0/3건(전면 WebFetch 차단) · snippet-verified: 3/3건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 12건 시도 → 3건 채택

## 2026-09-03 (일일 루프 #79)
- **신규 사례 3건** (WebFetch 전면 차단 환경, snippet-verified 전건; 한국 사례: 0건 — 2026-08-27~09-03 기간 네이버·카카오·토스·SKT·KT·우아한형제들·LY Corp·카카오뱅크·금융기관·의료기관 등 다수 쿼리 시도, 기수록 목록 외 신규 한국 1차 RAG 출처 미발굴)

| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-03-01 | arXiv:2609.02486 — ViSAR: 학습 불필요 적응형 k 검색으로 시각 문서 RAG 지연 58.7% 절감 | 프로덕션 아키텍처 (시각 문서 RAG) | 02-프로덕션-아키텍처.md |
| GL-2026-09-03-02 | arXiv:2609.02745 — Incremental Pooled LLM Eval: 프로덕션 검색 모델 선택 비용 효율적 평가 (JPMorganChase) | 프로덕션 아키텍처 (RAG 평가 방법론) | 02-프로덕션-아키텍처.md |
| GL-2026-09-03-03 | arXiv:2609.01601 — ACToR: 생성 중 임계 토큰 감지 기반 온디맨드 RAG (저장소 코드 생성) | 에이전트·툴유즈·MCP | 03-에이전트-툴유즈-MCP.md |

- **검증 결과**: 3건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2609.02486 ViSAR (2026-09-02)** [글로벌]: Adrien Mialland, Marc Plantevit, Julien Gallois, Céline Robardet. cs.CV. 시각 문서 RAG에서 쿼리 조건부 유사도 행렬로 검색 페이지 수 k를 동적 결정 — RAG 지연 최대 58.7% 절감, 정확도 동등·향상. 학습 불필요. → `02-프로덕션-아키텍처.md` Disco-RAG 다음(## 한국 환경 특이점 앞)에 추가
  2. **arXiv:2609.02745 Incremental Pooled LLM Eval (2026-09-02)** [글로벌·금융]: Max Nelson, Hanoz Bhathena, Aviral Joshi, Saket Sharma (JPMorganChase). 후보 시스템 union pool LLM 판단 + 증분 확장으로 관련성 판단 비용 절감. 62개 검색 구성 프로덕션 비교. 97% 쌍별 순위 보존. → `02-프로덕션-아키텍처.md` ViSAR 다음(## 한국 환경 특이점 앞)에 추가
  3. **arXiv:2609.01601 ACToR (2026-09-01)** [글로벌]: Kefeng Duan 외 10인. cs.SE·cs.AI·cs.CL. IEEE TSE 심사 중. 코드 생성 중 임계 토큰 감지 → 온디맨드 검색 트리거. RepoExec +8.4%, CoderEval +15.4%. → `03-에이전트-툴유즈-MCP.md` AgenticRag-R1 다음(## 이 도메인의 공통 패턴 앞)에 추가

- `sources.md`에 3개 출처 추가 (## 2026-09-03 일일 누적 추가 출처 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (2026-08-27~09-03 기간 기수록 목록 외 신규 한국 1차 출처 미발굴)
- **검증 결과**: URL 200 OK: 0/3건(전면 WebFetch 차단) · snippet-verified: 3/3건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 12건 시도 → 3건 채택
...
| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-09-01-01 | arXiv:2608.29622 — AgenticRag-R1: 메모리 스택과 계층 보상 에이전틱 멀티스텝 RAG RL | 에이전트·툴유즈·MCP | 03-에이전트-툴유즈-MCP.md |
| GL-2026-09-01-02 | arXiv:2608.28252 — Regime-Aware Portfolio: RAG+LLM 전문가 전환 시장 국면 포트폴리오 (EMNLP 2026 Findings) | 금융 산업 | 04-산업별-사례.md |

- **검증 결과**: 2건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2608.29622 AgenticRag-R1 (2026-08-30)** [글로벌]: Xinke Jiang 외 14인. 메모리 스택 + 세분화 행동 공간 + 계층 행동 인식 보상 + 정보 인식 궤적 거부로 장기 지평선 멀티스텝 RAG RL 학습 안정화. cs.MA·cs.AI. → `03-에이전트-툴유즈-MCP.md` MCite-RL 다음(## 이 도메인의 공통 패턴 앞)에 추가
  2. **arXiv:2608.28252 Regime-Aware Portfolio (2026-08-28, EMNLP 2026 Findings)** [글로벌]: Ahmad Asadi, Reza Safabakhsh (Amirkabir University of Technology). 이중 스트림 VAE + RAG 지식 베이스(과거 시장 상황-전문가 성과 쌍) + LLM 전문가 전환으로 비정상 금융 시장의 국면 전환 대응. 암호화폐·주식·외환 3개 도메인 최고 누적 수익률 달성. 단조성 속성 이론 증명. → `04-산업별-사례.md` 금융 > 글로벌 Self-Improving Financial RAG 다음(### 한국 앞)에 추가

- `sources.md`에 2개 출처 추가 (## 2026-09-01 일일 누적 추가 출처 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (2026-08-25~09-01 기간 기수록 목록 외 신규 한국 1차 출처 미발굴)
- **검증 결과**: URL 200 OK: 0/2건(전면 WebFetch 차단) · snippet-verified: 2/2건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 15건 시도 → 2건 채택

## 2026-08-31 (일일 루프 #76)
- **신규 사례 2건** (WebFetch 전면 차단 환경, snippet-verified 전건; 한국 사례: 0건 — 2026-08-24~31 기간 네이버·카카오·토스·SKT·KT·우아한형제들·LY Corp·삼성SDS·카카오뱅크·신한/하나/우리/국민은행 등 검색 결과 기수록 목록 외 신규 한국 1차 RAG 출처 미발굴)

| ID | 제목 | 도메인 | 파일 |
|----|------|--------|------|
| GL-2026-08-31-01 | arXiv:2608.21750 — FCPRAG: 파시지별 LoRA 주입 파라메트릭 RAG (EMNLP 2026) | 프로덕션 아키텍처 | 02-프로덕션-아키텍처.md |
| GL-2026-08-31-02 | arXiv:2608.26385 — Why RAGs Hallucinate: 패널티 인식 평가 + 지식-갭 카나리 | 프로덕션 아키텍처 (RAG 평가 방법론) | 02-프로덕션-아키텍처.md |

- **검증 결과**: 2건 전원 5점 자가검증 통과; snippet-verified (2개 이상 독립 출처)
  1. **arXiv:2608.21750 FCPRAG (2026-08-22, EMNLP 2026)** [글로벌]: Jinchang Zhu 외. 패시지별 LoRA 어댑터 + 경량 퓨전 컨트롤러(퓨전 스코어+믹싱 게이트+적응형 온도)로 멀티패시지 RAG 안정화. 인-컨텍스트 프롬프트 의존 없이 파라미터 공간에서 검색 지식 통합. 2WikiMultiHopQA F1 +4.65%p, CWQ F1 +7.55%p. → `02-프로덕션-아키텍처.md` PACE 다음(## 한국 환경 특이점 앞)에 추가
  2. **arXiv:2608.26385 Why RAGs Hallucinate (2026-08-26)** [글로벌]: Alden Do Rosario, Hussein Younes, Felipe Pires. 패널티 인식 평가(정답+1/오답−4/기권0) + 지식-갭 카나리 + 실패 귀인 파이프라인. 상업용 RAG 3개 × 1,000 SimpleQA-Verified × 3회 반복 실험. 기권율 시스템 간 50배 이상 차이 — 단순 정확도로는 구별 불가. → `02-프로덕션-아키텍처.md` FCPRAG 다음에 추가

- `sources.md`에 2개 출처 추가 (## 2026-08-31 일일 누적 추가 출처 섹션 신설, 파일 최상단)
- **한국 사례**: 0건 (2026-08-24~31 기간 기수록 목록 외 신규 한국 1차 출처 미발굴)
- **검증 결과**: URL 200 OK: 0/2건(전면 WebFetch 차단) · snippet-verified: 2/2건 · 단언 톤다운: 0건 · 중복 폐기: 0건 · 발굴 시도 → 최종 채택: 약 12건 시도 → 2건 채택
