...
- **Google Gemini Enterprise Agent Platform — Sufficient Context Agent** (2026-06): Google Research가 Gemini Enterprise Agent Platform에 Agentic RAG를 추가. 핵심은 **Sufficient Context Agent**: 검색된 문서가 질문 답변에 충분한지 Gemini 1.5 Pro 기반 분류기(93% 정확도)로 판단하고, 부족하면 추가 검색을 반복. 단일 retrieval로 처리 불가능한 multi-hop·multi-source 쿼리("Project X에 사용된 서버 스펙"처럼 문서 A에서 ID 발견 → 문서 B에서 스펙 조회하는 2단계 query)를 자율 처리. FramesQA 기준(824쿼리·2,676 PDF) 평가에서 표준 RAG 대비 factuality 정확도 +34% 향상. 출처: [Google Research Blog — Unlocking dependable responses with Gemini Enterprise Agent Platform's Agentic RAG](https://research.google/blog/unlocking-dependable-responses-with-gemini-enterprise-agent-platforms-agentic-rag/) (2026-06)
